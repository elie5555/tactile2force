...
from models.m3_parameter_free_linear_dampned import M3
...
def run_m3(n_train, n_test, finger, patch_type):
    model = M3()
    model_name = 'M3'
    ...
    print(train_MSE)
    print(test_MSE)
    rel_err = get_rel_error(y_val, test_fit, threshold=0.5)
    print('relative error is: ', rel_err)
    rel_error_inferior_to = get_rel_error_inferior_to(y_val, test_fit)
    print('relative error is: ', rel_error_inferior_to)

    # Timestamp and save train and test metadata
    now = datetime.now()
    save_performance(now, model_name, train, val, train_MSE, test_MSE, train_norm_MSE, test_norm_MSE, rel_err, rel_error_inferior_to, standardization_dict)
...
if __name__ == '__main__':
    ...
    finger = 'index'
    patch_type = 'tip'
    run_m3(n_train, n_test, finger, patch_type)