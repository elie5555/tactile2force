...
import models.m6_cnn_2 as net
#import models.m6_cnn_2 as net2
...
def run_m7(n_train, n_test, finger, patch_type):
    model_name = 'M7'
    model = net.SimpleCNN(patch_type)
    print(model.parameters())

    train, val, x_train, x_val, standardization_dict = tm.prep_data_cnn(n_train, n_test, finger, patch_type, 256, True, 10)
    std_tactile = standardization_dict["tactile_scale"]
    std_force = standardization_dict["force_scale"]

    train_loss, val_loss = net.train_network(model, x_train, x_val, num_epochs=120, learning_rate=0.00025)
    plt.close()
    #train_loss, val_loss = net2.train_network(model2, x_train, x_val, num_epochs=300, learning_rate=0.00025)
    train_fit = net.predict(model, x_train.dataset.data)
    #train_fit2 = net2.predict(model2, x_train.dataset)
    test_fit = net.predict(model, x_val.dataset.data)
    #test_fit2 = net2.predict(model2, x_val.dataset.data)
...
if __name__ == '__main__':
    ...
    finger = 'index'
    patch_type = 'tip'
    run_m7(n_train, n_test, finger, patch_type)